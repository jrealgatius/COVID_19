...
ggsave("plot_esp.png",device="png")
# 
...
list(list_plots, names(list_plots)) %>% 
        pwalk( ~ ggsave(plot=.x,filename=paste0(.y,".png"),device = "png") )
...
library(grid)
library(gridExtra)
gridExtra::grid.arrange(plot_resta,list_plots$Madrid,ncol=2,
                        top = textGrob("Daily QC: Blue",gp=gpar(fontsize=20,font=3)),
                        common.legend = TRUE, legend="bottom")


plot_resta<-plot_resta + 
  labs(title = "Resto de España", subtitle = "",caption = "") + 
  scale_y_continuous(labels = scales::percent_format(accuracy = 0.1), breaks = seq(0,0.14,by=0.02),limits = c(0,0.14))

plot_Madrid<-plot_Madrid + 
  labs(title = "Madrid", subtitle ="",caption = "") + 
  scale_y_continuous(labels = scales::percent_format(accuracy = 0.1), breaks = seq(0,0.14,by=0.02),limits = c(0,0.14))

library(ggpubr)
ggarrange(plot_Madrid,plot_resta,ncol=2, 
          common.legend = TRUE, legend="bottom")
